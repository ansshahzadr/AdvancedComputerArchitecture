...
static void matvec_sse()
{
        ...
        for (int row = 0; row < SIZE; row+=4){
            ...
            for (int col = 0; col < SIZE; col+=4){
                ...
                __m128 mat1 = _mm_load_ps (&mat_a[col+SIZE*(row+1)]);
                __m128 mat2 = _mm_load_ps (&mat_a[col+SIZE*(row+2)]);
                __m128 mat3 = _mm_load_ps (&mat_a[col+SIZE*(row+3)]);
                res = _mm_add_ps(res,_mm_mul_ps(mat, vec));
                res1 = _mm_add_ps(res1,_mm_mul_ps(mat1, vec));
                res2 = _mm_add_ps(res2,_mm_mul_ps(mat2, vec));
                res3 = _mm_add_ps(res3,_mm_mul_ps(mat3, vec));
            }
            res = _mm_hadd_ps(res,res1);
            res2 = _mm_hadd_ps(res2,res3);
            res = _mm_hadd_ps(res,res2);
